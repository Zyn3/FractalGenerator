...
namespace FractalGenerator
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                {
                    seed = float.Parse(args[++i], CultureInfo.InvariantCulture);
                }
                else if (args[i] == "--fractal" && i + 1 < args.Length)
                {
                    fractalType = (FractalType)int.Parse(args[++i]);
                }
            }

            var bitmap = new Bitmap(width, height);
            Fractal fractal = new Fractal(bitmap, seed, fractalType);
            fractal.Generate();

            string filename = $"Fractal_{fractalType}_{seed.ToString(CultureInfo.InvariantCulture)}.bmp";
            bitmap.Save(filename, System.Drawing.Imaging.ImageFormat.Bmp);
            Console.WriteLine($"Fractal image saved as {filename}");
        }
    }
    ...
    class Fractal
    {
        ...
        public Fractal(Bitmap bitmap, float seed, FractalType fractalType)
        {
            _bitmap = bitmap;
            _seed = seed;
        ...
        {
            int maxIterations = 1000;
            Complex c = new Complex(_seed, _seed);
